...
def main():
    ...
    # model = unet_bn_t(pretrained_weights=None, input_size=(256, 256, 1), depth=4, n_base_filters=64, optimizer=Adam, activation=LeakyReLU, batch_normalization=True, initial_learning_rate=5e-4, loss_function=dice_coefficient_loss)
    model = unet_bn_t(**net_conf)
    # model.load_weights("result/unet_bnt-07-1.00.hdf5")
    results = model.fit_generator(generator=training_generator, validation_data=validation_generator, epochs=50, callbacks=[model_checkpoint, early_stoping, tensorboard])
