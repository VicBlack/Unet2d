...
def unet_bn_deconv_dp_2d(pretrained_weights=None, input_size=(256, 256, 1), depth=4, n_base_filters=64, optimizer=Adam, activation=LeakyReLU, batch_normalization=True, initial_learning_rate=5e-4, loss_function=dice_coefficient_loss, multi_gpu_num=0):
    x = Input(input_size)
    # 输入层
    inputs = x
    skiptensors = []  # 用于存放下采样中，每个深度后的tensor，以供之后级联使用
...
def GetNet(model_type='unet_bn_upsampling_2d', net_conf=None):
    ...
    if model_type == 'unet_bn_deconv_dp_2d':
        return unet_bn_deconv_dp_2d(**net_conf)
...
if __name__=='__main__':
    ...
    # unet_bn_deconv_dp_2d = GetNet('unet_bn_deconv_dp_2d')
